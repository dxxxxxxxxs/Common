...
export default class ListItem extends cc.Component {
    ...
    onLoad() {
        ...
        if (this.selectedMode == SelectedType.SWITCH) {
            if (!this.selectedFlag) {
                cc.warn(this.node.name + "'s ListItem unset selectedFlag, selectedMode will fallback to NONE.");
                this.selectedMode = SelectedType.NONE;
                return;
            }
            let com: cc.Sprite = this.selectedFlag.getComponent(cc.Sprite);
            if (!com) {
                cc.warn(this.node.name + "'s selectedFlag unset cc.Sprite, selectedMode will fallback to NONE.");
                this.selectedMode = SelectedType.NONE;
                return;
            }
