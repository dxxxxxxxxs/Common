...
export default class List extends cc.Component {
    ...
    //虚拟列表缓冲区倍率（按视窗尺寸扩展，降低快速滑动时的白边感）
    @property({ type: cc.Float })
    private _virtualBufferScale: number = .5;
    @property({
        type: cc.Float,
        range: [0, 2, .25],
        tooltip: CC_DEV && '虚拟列表视窗缓冲倍率（值越大，预渲染越多）',
        slide: true,
        visible() { return this.virtual; }
    })
    set virtualBufferScale(val: number) {
        if (val >= 0)
            this._virtualBufferScale = val;
    }
    get virtualBufferScale() {
        return this._virtualBufferScale;
    }
    //刷新频率
    ...
    private _customSizeCacheKeys: number[] = [];
    private _customSizeCachePrefix: number[] = [];
    ...
    _init() {
        ...
        if (!t._layout) {
            cc.error(t.node.name + "'s content unset cc.Layout!");
            return;
        }
        ...
        if (t.templateType == TemplateType.PREFAB && !t.tmpPrefab) {
            cc.error(t.node.name + "'s List unset tmpPrefab!");
            return;
        }
        if (t.templateType == TemplateType.NODE && !t.tmpNode) {
            cc.error(t.node.name + "'s List unset tmpNode!");
            return;
        }
        ...
        t._customSizeCacheKeys = [];
        t._customSizeCachePrefix = [];
    ...
    _onScrolling(ev: cc.Event = null) {
        ...
        if (this._virtual) {
            let buffer = this._getVirtualBuffer();
            if (this._sizeType) {
                vTop += buffer;
                vBottom -= buffer;
            } else {
            ...
            if (this._customSize) {
                curId = this._estimateCustomSizeStartId(vTop, vRight, vBottom, vLeft);
    ...
    _refreshCustomSizeCache() {
        this._customSizeCacheKeys = [];
        this._customSizeCachePrefix = [];
        if (!this._customSize)
            return;
        let keys = Object.keys(this._customSize)
            .map(id => parseInt(id))
            .filter(id => !isNaN(id))
            .sort((a, b) => a - b);
        let prefix: number[] = [];
        ...
        for (let n: number = 0; n < keys.length; n++) {
            total += this._customSize[keys[n]];
            prefix.push(total);
        }
        this._customSizeCacheKeys = keys;
        this._customSizeCachePrefix = prefix;
    }
    _getCustomSizeCacheIndex(listId: number) {
        let left = 0;
        let right = this._customSizeCacheKeys.length;
        while (left < right) {
            let mid = (left + right) >> 1;
            if (this._customSizeCacheKeys[mid] < listId)
                left = mid + 1;
            else
                right = mid;
        }
    ...
    _getVirtualBuffer() {
        return (this._sizeType ? this.node.height : this.node.width) * this._virtualBufferScale;
    }
    _estimateCustomSizeStartId(vTop: number, vRight: number, vBottom: number, vLeft: number) {
        let unit = this._sizeType ? (this._itemSize.height + this._lineGap) : (this._itemSize.width + this._columnGap);
        if (unit <= 0)
            return 0;
        let result = 0;
        switch (this._alignCalcType) {
            case 1:
                result = Math.floor((vLeft - this._leftGap) / unit);
                break;
            case 2:
                result = Math.floor((-vRight - this._rightGap) / unit);
                break;
            case 3:
                result = Math.floor((-vTop - this._topGap) / unit);
                break;
            case 4:
                result = Math.floor((vBottom - this._bottomGap) / unit);
                break;
        }
        result = (result - 2) * this._colLineNum;
    ...
    _getFixedSize(listId: number) {
        ...
        let count: number = this._getCustomSizeCacheIndex(listId);
        let fixed: number = count > 0 ? this._customSizeCachePrefix[count - 1] : 0;
        return {
            val: fixed,
            count: count,
        }
    ...
    _onTouchStart(ev: any, captureListeners: any) {
        if ((this._scrollView as any)['hasNestedViewGroup'](ev, captureListeners))
            return;
    ...
    _onTouchCancelled(ev: any, captureListeners: any) {
        let t = this;
        if ((t._scrollView as any)['hasNestedViewGroup'](ev, captureListeners) || ev.simulate)
            return;
    ...
    _onItemAdaptive(item) {
        ...
        let defaultVal = this._sizeType ? this._itemSize.height : this._itemSize.width;
        if (val != defaultVal || (this._customSize && this._customSize[item._listId] != null)) {
            let changed = false;
            if (val == defaultVal) {
                if (this._customSize && this._customSize[item._listId] != null) {
            ...
            if (this._customSize && !Object.keys(this._customSize).length) {
                this._customSize = null;
            }
            if (changed) {
                this._refreshCustomSizeCache();
                this._resizeContent();
    ...
    aniDelItem(listId: number, callFunc: Function, aniType: number) {
        ...
        listItem.showAni(aniType, () => {
            ...
            if (t._customSize) {
                ...
                t._customSize = Object.keys(newCustomSize).length ? newCustomSize : null;
                t._refreshCustomSizeCache();
            }
            //后面的Item向前怼的动效
            let sec: number = .2333;
